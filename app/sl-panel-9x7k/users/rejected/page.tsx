...
export default async function RejectedUsersPage({ searchParams }: PageProps) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-white/60 mt-1">
                        {pagination.total} utilisateur{pagination.total > 1 ? 's' : ''} rejetés
                    </p>
                </div>
            </div>
            ...
            <UsersTable users={formattedUsers} pagination={pagination} basePath="/sl-panel-9x7k/users/rejected" />
        </div>
    );
}
