...
export function FilterBar({ filters, configs, onUpdate, onReset, className = '' }: FilterBarProps) {
                    ...
                    <div className={`relative filter-dropdown-container ${openFilter === config.key ? 'z-50' : 'z-30'}`}>
                        ...
                        >
                            <span className="text-sm font-medium">
                                {config.type === 'number' && filters[config.key] > 0
                                    ? `${filters[config.key]}+ ${config.label}`
                                    : (filters[config.key] || config.label)}
                            </span>
                            <ChevronDown className={`w-4 h-4 transition-transform ${openFilter === config.key ? 'rotate-180' : ''}`} />
                        </button>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-xs font-medium text-gray-700 mb-2">
                                            {config.defaultValue === config.max ? 'Max' : 'Valeur'}: {filters[config.key]?.toLocaleString()} {config.unit}
                                        </label>
                                        <input
                                            type="range"
                                            min={config.min || 0}
